...
#Server.R app for the NextWordProjection App
...
qd <- readRDS("NextWordPrediction/ngram4.rds")
tg<- readRDS("NextWordPrediction/ngram3.rds")
bg <- readRDS("NextWordPrediction/ngram2.rds")
...
functionBigram <- function(the_word) {
  ...
  if (identical(character(0),as.character(head(bg[bg$w1 == the_word[1], 2], 1)))) {
    # testing print(bg$w1)
    message<<-"If no word found use the as it is the most common word in English" 
